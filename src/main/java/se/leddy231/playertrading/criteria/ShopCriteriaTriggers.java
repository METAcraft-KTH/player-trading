...
public class ShopCriteriaTriggers {
	...
	public static <T extends CriterionTrigger<?>> T register(String string, T criterionTrigger) {
		return Registry.register(
				...
				Identifier.fromNamespaceAndPath(PlayerTrading.MODID, string),
				criterionTrigger
		);
	}
	...
	private static ContextKeySet registerKeySet(String string, Consumer<ContextKeySet.Builder> consumer) {
		...
		Identifier id = Identifier.fromNamespaceAndPath(PlayerTrading.MODID, string);
		ContextKeySet contextKeySet2 = LootContextParamSetsAccessor.getRegistry().put(id, contextKeySet);
