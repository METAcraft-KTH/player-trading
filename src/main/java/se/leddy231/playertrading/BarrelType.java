...
public enum BarrelType {
    NONE(0, "None", null), SHOP(1, "Shop", "{shop}"), OUTPUT(2, "Output", "{output}"), STOCK(3,
                                                                                             "Stock",
                                                                                             "{stock}"
    ), PERMANENT(4, "Permanent shop", "{permanent shop}"), ADMIN(
            5,
            "Admin shop",
            "{admin shop}"
    ), STORAGE(6, "Storage", "{storage}"), SINGLEUSE(7, "Single use", "{single use}");

    private final int id;
    private final String typeName;
    private final String signTag;

    BarrelType(int id, String typeName, String signTag) {
        this.id = id;
